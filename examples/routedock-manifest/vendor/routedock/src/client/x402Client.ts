...
export class X402Client {
  ...
  async pay(url: string, manifest: RouteDockManifest): Promise<PaymentResult> {
    ...
    return withRetry(async () => {
      ...
      // PATCHED — not vendored verbatim, see vendor/routedock/PROVENANCE.md.
      // The real @x402/core server (server/index.js: `"PAYMENT-REQUIRED": encodePaymentRequiredHeader(...)`)
      // sets this header as `PAYMENT-REQUIRED`, not `X-Payment-Requirements`.
      // Confirmed against both a live probe of the real nirium-agent.fly.dev
      // endpoint and a local run of the actual, unmodified x402Serve() in
      // this example — both return `payment-required` (HTTP headers are
      // case-insensitive). RouteDock's original line here reads only
      // `X-Payment-Requirements`, which never matches, so `pay()` cannot
      // complete an x402 payment against a real @x402/core server as
      // published today. Checking both keeps this working if RouteDock
      // fixes/aliases it upstream.
      const reqHeader = init.headers.get('PAYMENT-REQUIRED') ?? init.headers.get('X-Payment-Requirements')
      if (!reqHeader) {
        throw new RouteDockManifestError('402 response missing PAYMENT-REQUIRED header')
      }
      ...
      // PATCHED — see the matching note above; the real server sets
      // `PAYMENT-RESPONSE` (server/index.js: `return { "PAYMENT-RESPONSE": encoded }`).
      const responseHeader = settled.headers.get('PAYMENT-RESPONSE') ?? settled.headers.get('X-Payment-Response')
      if (responseHeader) {
        try {
